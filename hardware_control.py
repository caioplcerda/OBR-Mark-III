# hardware_control.py
# Open-loop por padrão (reto confiável), com fallback/upgrade para encoders depois.
# Flags de polaridade por roda e deadband de 50%.

import time
import threading

# =========================
#   Backends GPIO (RPi.GPIO -> lgpio fallback)
# =========================
class _PWMBase:
...
class _RPiGPIOBackend(_GPIOBackendBase):
    ...
    def setup(self, pin, mode, pull_up_down=None):
        if pull_up_down is None: self._GPIO.setup(pin, mode)
        else: self._GPIO.setup(pin, mode, pull_up_down=pull_up_down)
    def output(self, pin, level):
        self._GPIO.output(pin, self._GPIO.HIGH if level else self._GPIO.LOW)
    def input(self, pin): return self._GPIO.input(pin)
    class _PWM(_PWMBase):
        def __init__(self, GPIO, pin, freq): self._pwm = GPIO.PWM(pin, freq)
    ...
    def PWM(self, pin, freq_hz): return self._PWM(self._GPIO, pin, freq_hz)
    def add_event_detect(self, pin, edge, callback): self._GPIO.add_event_detect(pin, edge, callback=callback)
...
class _LGPIOBackend(_GPIOBackendBase):
    def __init__(self, chip_index=0):
        ...
        self._alerts = {}
    def _claim_out(self, pin, init=0): self._lgpio.gpio_claim_output(self._h, pin, init)
    def _claim_in(self, pin, pud=None):
        flags = 0
        if pud == self.PUD_UP: flags |= self._lgpio.SET_PULL_UP
        self._lgpio.gpio_claim_input(self._h, pin, flags)
    def setup(self, pin, mode, pull_up_down=None):
        if mode == self.OUT: self._claim_out(pin, 0)
        else: self._claim_in(pin, pull_up_down)
    def output(self, pin, level): self._lgpio.gpio_write(self._h, pin, 1 if level else 0)
    def input(self, pin): return self._lgpio.gpio_read(self._h, pin)
    class _PWM(_PWMBase):
        ...
        def start(self, duty): self.ChangeDutyCycle(duty)
    ...
    def PWM(self, pin, freq_hz): return self._PWM(self._lgpio, self._h, pin, freq_hz)
    def add_event_detect(self, pin, edge, callback):
        ...
        def _cb(_chip,_gpio,level,_tick):
            try:
                if level==1: callback(pin)
            except Exception: pass
        self._alerts[pin]=_cb; self._lgpio.set_alert_func(self._h, pin, _cb)
    def remove_event_detect(self, pin):
    ...
    def cleanup(self):
        try:
            for pin in list(self._alerts.keys()): self.remove_event_detect(pin)
        except Exception: pass
...
def _make_backend(prefer="rpi"):
    if prefer=="rpi":
        try: return _RPiGPIOBackend(), "rpi"
        except Exception: pass
    return _LGPIOBackend(), "lgpio"

# =========================
#   PID genérico (correção direcional)
# =========================
class PIDController:
...
class HardwareControl:
    ...
    Use set_open_loop(False) quando quiser ativar controle por encoder já calibrado.
    """
    ...
    def _safe_setup_io(self):
        def base_setup():
            ...
            self.GPIO.output(self.STBY, 1)  # STBY alto (ligado em HW)
        ...
        try:
            ...
            if _BASE_ERR in str(e): self._switch_to_lgpio()
            else: raise
        except Exception:
    ...
    def _apply_deadband(self, cmd):
        ...
        a=abs(cmd)
        if a<self.MIN_DUTY: return self.MIN_DUTY*(1.0 if cmd>0 else -1.0)
        return cmd
    ...
    def set_motor_speed(self, base_speed, error):
        ...
        # deadband para garantir movimento mesmo com “reto perfeito”
    ...
    def _control_loop(self):
        ...
        while self._ctrl_running:
            ...
            if not self.OPEN_LOOP and self._encoders_ok:
                now=time.time(); dt=now-self._last_meas_t
                if dt<=0: dt=period
                ...
                inst_l=float(dt_l)/dt; inst_r=float(dt_r)/dt
    ...
    # ---------- servos ----------
    @staticmethod
    def _us_to_duty(us, period_ms=20.0): return max(0.0, min(100.0, (us/1000.0)/period_ms*100.0))
    def _servo_write_us(self, index, us):
        i=index-1
        if not (0<=i<4) or not self._servo_ready[i]: return False
        self._servo_pwm[i].ChangeDutyCycle(self._us_to_duty(int(us))); return True
    def set_servo(self, index, pos, smooth_ms=0):
        i=index-1
        if not (0<=i<4) or pos not in ("A","B"): return False
        ...
        if smooth_ms>0:
            ...
            for t in range(steps+1):
                ...
                self._servo_write_us(index,u); time.sleep(0.02)
        else:
            self._servo_write_us(index,target)
        return True
    def set_servos(self, positions=('A','A','A','A'), smooth_ms=0):
        for idx,p in enumerate(positions, start=1): self.set_servo(idx,p,smooth_ms=smooth_ms)
    def set_servo_us(self, index, us): return self._servo_write_us(index,int(us))
    def _init_servos(self):
        for i,pin in enumerate(self.SERVO_PINS):
            try:
                self.GPIO.setup(pin, self.GPIO.OUT)
                pwm=self.GPIO.PWM(pin, self.SERVO_FREQ_HZ); pwm.start(0)
                self._servo_pwm[i]=pwm; self._servo_ready[i]=True

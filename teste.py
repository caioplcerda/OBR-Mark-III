    ...
    def _loop(self):
        try:
            while self.running:
                ...
                # compressão do frame para economizar memória na UI
                ok, jpg = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), 60])
                SHARED_STATE["last_frame"] = jpg.tobytes() if ok else None
                SHARED_STATE["speeds"] = {
                ...
                def _toggle(channel):
                    if GPIO.input(p) == GPIO.LOW:
                        if robot.running:
                            ...
                            robot.start()
                return _toggle
            GPIO.add_event_detect(pin, GPIO.FALLING, callback=_make_toggle(pin), bouncetime=300)
